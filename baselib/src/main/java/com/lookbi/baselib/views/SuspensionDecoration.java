...
public class SuspensionDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int COLOR_TITLE_BG = Color.parseColor("#DBDCDE");
    private static int COLOR_TITLE_FONT = Color.parseColor("#130F18");
    ...
    public SuspensionDecoration(Context context, List<String> classifies) {
        super();
        this.classifies = classifies;
        mPaint = new Paint();
        mBounds = new Rect();
        //dp转px
        mTitleHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 43, context.getResources().getDisplayMetrics());
...
//        Log.d(TAG, "onDrawOver");
        ...
        if ((pos + 1) < classifies.size()) {//防止数组越界（一般情况不会出现）
            if (null != tag && !tag.equals(classifies.get(pos + 1))) {//当前第一个可见的Item的tag，不等于其后一个item的tag，说明悬浮的View要切换了
